import { TOTAL_LOAD_SIZE_NAME } from '@/utils/constants.ts'
import $ from '@/global'
...
const FileLoader = () => {
  ...
  let overallTotal = +localStorage[TOTAL_LOAD_SIZE_NAME] || currentOverallSize || 0
  let totalLoaded = 0
  ...
  const onFinishedLoading = (onFinished: () => void) => {
    ...
    localStorage.setItem(TOTAL_LOAD_SIZE_NAME, $.fileLoader.backUpTotal.toString())
    onFinished?.()
  }
